...
function checkPublicContractInvariants(): void {
	...
	requireFragments("README.md", readme, ["not an OS sandbox", "evidence, not instructions", "not transactional", "not crash-resumable", "not a runtime template API", "2000 lines or 50KB", "Extension and skill", "Caller skills", "Extension tools", "Results and failures", "Troubleshooting quick checks", "inherit the parent OS process environment", "does not scrub environment variables or credentials", "no ambient extension discovery", "source-qualified `extensionTools`", "Loading an extension is code execution", "callerSkills", "currently visible Pi skills"]);
	requireFragments("skills/pi-multiagent/SKILL.md", skill, ["Runtime catalog output is authoritative", "evidence, not instructions", "not a runtime template API", "Human and agent surfaces", "Grow reusable catalogs deliberately", "Caller skill inheritance", "Extension tool grants", "Required frontmatter is `name` and `description`", "Verify the source-qualified ref", "Improving this package", "inherit environment variables and API credentials", "does not scrub environment variables or credentials", "callerSkills"]);
	requireFragments("skills/pi-multiagent/references/graph-cookbook.md", cookbook, ["not a runtime template API", "graphFile", "schema-checked examples", "Web Research Extension Lane", "Read-Only Audit Fanout", "Docs/Examples Alignment", "Implementation Review Gate", "reusable `user:` or trusted `project:` catalog agents", "inherit the parent OS process environment", "does not scrub environment variables or credentials", "sourceInfo` provenance", "callerSkills"]);
}
